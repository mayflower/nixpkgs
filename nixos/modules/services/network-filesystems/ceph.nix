...
let
  ...
  makeService = (daemonType: daemonId: clusterName: ceph: extraServiceConfig: {
    ...
    serviceConfig = {
      ...
      ExecStart = "${ceph.out}/bin/${if daemonType == "rgw" then "radosgw" else "ceph-${daemonType}"} -f --cluster ${clusterName} --id ${daemonId} --setuser ceph --setgroup disk";
    } // extraServiceConfig
  ...
  config = mkIf config.services.ceph.enable {
    ...
    systemd.services = let
      services = []
        ...
        ++ optional cfg.osd.enable (makeServices "osd" cfg.osd.daemons { StartLimitBurst = "30"; RestartSec = "20s"; PrivateDevices = "no"; })
