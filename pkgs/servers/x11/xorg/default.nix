...
lib.makeScope newScope (self: with self; {
  ...
  xcbutil = callPackage ({ stdenv, pkgconfig, fetchurl, gperf, libxcb, m4 }: stdenv.mkDerivation {
    name = "xcb-util-0.4.0";
    builder = ./builder.sh;
    src = fetchurl {
    ...
    buildInputs = [ gperf libxcb ];
    meta.platforms = stdenv.lib.platforms.unix;
  }) {};

  xcbutilcursor = callPackage ({ stdenv, pkgconfig, fetchurl, gperf, libxcb, xcbutilimage, xcbutilrenderutil, m4 }: stdenv.mkDerivation {
    ...
    hardeningDisable = [ "bindnow" "relro" ];
    nativeBuildInputs = [ pkgconfig m4 ];
    buildInputs = [ gperf libxcb xcbutilimage xcbutilrenderutil ];
    meta.platforms = stdenv.lib.platforms.unix;
  }) {};

  xcbutilerrors = callPackage ({ stdenv, pkgconfig, fetchurl, gperf, libxcb, xcbproto, m4 }: stdenv.mkDerivation {
    ...
    hardeningDisable = [ "bindnow" "relro" ];
    nativeBuildInputs = [ pkgconfig m4 ];
    buildInputs = [ gperf libxcb xcbproto ];
    meta.platforms = stdenv.lib.platforms.unix;
  }) {};
  ...
  xcbutilrenderutil = callPackage ({ stdenv, pkgconfig, fetchurl, gperf, libxcb, m4 }: stdenv.mkDerivation {
  ...
  xcbutilwm = callPackage ({ stdenv, pkgconfig, fetchurl, gperf, libxcb, m4 }: stdenv.mkDerivation {
