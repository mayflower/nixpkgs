{ stdenv, runCommand, nettools, bc, perl, kmod, libssl, writeTextFile, ubootChooser }:
...
stdenv.mkDerivation ((drvAttrs config stdenv.platform (kernelPatches ++ nativeKernelPatches) configfile) // {
  ...
  nativeBuildInputs = [ perl bc nettools libssl ] ++ optional (stdenv.platform.uboot != null)
